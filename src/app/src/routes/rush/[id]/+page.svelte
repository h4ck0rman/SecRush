...
<script>
  ...
  async function handleMonacoReady() {
    ...
    try {
      ...
      const {id , code, difficulty, language, vulnerableLine} = data;
      solution = code;

      codeUpdate(code, language);
      solutionHighlight([
          { line: vulnerableLine, colour: 'green' }
      ]);
  ...
  <div class="flex flex-col md:flex-row flex-1 overflow-hidden">
    ...
    <footer class={`bg-black px-4 pb-4 md:order-first md:w-1/4 md:py-20 md:h-full h-auto flex flex-col relative transition-all duration-300 ${showSolution ? 'fixed inset-x-0 bottom-0 h-4/5' : 'relative h-auto'}`}>
      <div class="flex flex-col space-y-1 md:space-y-4 h-full">
          ...
          <div class="w-full pt-4 flex flex-col items-start md:py-10">
              <button
                  type="button"
              ...
              {#if showSolution}
                  <div class="w-full font-mono whitespace-pre-line bg-gray-900 mt-4 p-4 overflow-y-auto max-h-[60vh]">
                      {solution}
                  </div>
              {/if}
          </div>
